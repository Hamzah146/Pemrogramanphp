<?php 
    class Produk{
        ...
        public function simpan($data){
            $sql = "INSERT INTO produk(kode,nama,harga_produk,stok,min_stok,jenis_barang_id)
                    VALUES (?,?,?,?,?,?)";
        ...
        public function ubah($data){
            $sql = "UPDATE produk SET kode=?, nama=?, harga_produk=?, stok=?, min_stok=?, jenis_produk_id=? WHERE id=?";
            // prepare statement PDO
            $ps = $this->dbh->prepare($sql); 
            $ps->execute($data);
        }
